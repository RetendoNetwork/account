...
import { IRNID, IRNIDMethods } from '@/types/mongoose/rnid';
...
export async function sendConfirmationEmail(rnid: mongoose.HydratedDocument<IRNID, IRNIDMethods>): Promise<void> {
	const options = {
...
export async function sendForgotPasswordEmail(rnid: mongoose.HydratedDocument<IRNID, IRNIDMethods>): Promise<void> {
	const tokenOptions = {
		system_type: 0xF, // * API
