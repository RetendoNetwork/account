...
import { IRNID, IRNIDMethods, RNIDModel } from '@/types/mongoose/rnid';
...
const RNIDSchema = new Schema<IRNID, RNIDModel, IRNIDMethods>({
	deleted: {
		type: Boolean,
		default: false
	},
...
*/
RNIDSchema.method('generatePID', async function generatePID(): Promise<void> {
...
RNIDSchema.method('generateEmailValidationCode', async function generateEmailValidationCode(): Promise<void> {
...
RNIDSchema.method('generateEmailValidationToken', async function generateEmailValidationToken(): Promise<void> {
...
RNIDSchema.method('updateMii', async function updateMii({ name, primary, data }: { name: string; primary: string; data: string; }): Promise<void> {
	this.mii.name = name;
...
RNIDSchema.method('generateMiiImages', async function generateMiiImages(): Promise<void> {
...
RNIDSchema.method('scrub', async function scrub() {
	// * Remove all personal info from a RNID
	...
	if (this.connections?.stripe?.subscription_id) {
		try {
			if (stripe) {
				...
				logger.warn(`SCRUBBING USER DATA FOR USER ${this.username}. HAS STRIPE SUBSCRIPTION ${this.connections.stripe.subscription_id}, BUT STRIPE CLIENT NOT ENABLED. SUBSCRIPTION NOT CANCELED`);
			}
		} catch (error) {
			logger.error(`ERROR REMOVING ${this.username} STRIPE SUBSCRIPTION. ${error}`);
		}
	}
...
RNIDSchema.method('hasPermission', function hasPermission(flag: RNIDPermissionFlag): boolean {
	return (this.permissions & flag) === flag;
});

RNIDSchema.method('addPermission', function addPermission(flag: RNIDPermissionFlag): void {
	this.permissions |= flag;
});

RNIDSchema.method('clearPermission', function clearPermission(flag: RNIDPermissionFlag): void {
	this.permissions &= ~flag;
});

export const RNID = model<IRNID, RNIDModel>('RNID', RNIDSchema);