...
router.post('/', async (request, response) => {
	...
	if (grant_type === 'password') {
		rnid = await database.getUserByUsername(username);
		if (!rnid) {
			return response.status(400).json({
				app: 'api',
				status: 400,
		...
		const hashedPassword = util.nintendoPasswordHash(password, rnid.get('pid'));
		...
		rnid = await database.getUserBearer(refresh_token);
		if (!rnid) {
			return response.status(400).json({
				app: 'api',
				status: 400,
