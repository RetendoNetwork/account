...
async function sendConfirmationEmail(rnid) {
	await mailer.sendMail({
		to: rnid.get('email.address'),
		subject: '[Retendo Network] Please confirm your email address',
		username: rnid.get('username'),
		confirmation: {
			href: `https://api.retendo.online/v1/email/verify?token=${rnid.get('identification.email_token')}`,
			code: rnid.get('identification.email_code')
		},
		text: `Hello ${rnid.get('username')}! \r\n\r\nYour Retendo Network ID activation is almost complete. Please click the link to confirm your e-mail address and complete the activation process: \r\nhttps://api.pretendo.cc/v1/email/verify?token=${rnid.get('identification.email_token')} \r\n\r\nYou may also enter the following 6-digit code on your console: ${rnid.get('identification.email_code')}`
	});
