...
/* async function sendConfirmationEmail(rnid) {
	await mailer.sendMail({
		...
		html: `Hello,<br><br>Your Retendo Network ID activation is almost complete.<br><br>Please click the link below to confirm your e-mail address and complete the activation process.<br><br>https://api.innoverse.club/v1/email/verify?token=${rnid.get('identification.email_token')}<br><br>If you are unable to connect to the above URL, please enter the following confirmation code on the device to which your Rentendo Network ID is linked.<br><br>&lt;&lt;Confirmation code: ${rnid.get('identification.email_code')}&gt;&gt;`
	});
}
